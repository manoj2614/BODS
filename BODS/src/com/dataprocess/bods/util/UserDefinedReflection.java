...
/**
 * The Class UserDefinedReflection.
 */
public final class UserDefinedReflection {
	...
	public Object invokeMethod(Object o, String methodName, Object[] args) {
		...
		if (args != null) {
			...
			for (int len = 0; len < args.length; len++) {
				...
				} else if (args[len] instanceof BigDecimal) {
					try {
						args[len] = new Long(
								((BigDecimal) args[len]).longValueExact());
						argumentArray[len] = long.class;
					} catch (ArithmeticException e) {
						args[len] = new Double(
								((BigDecimal) args[len]).doubleValue());
						argumentArray[len] = double.class;
					}
